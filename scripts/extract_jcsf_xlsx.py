#!/usr/bin/env python3
"""Extract rows from JCSF v7_public.xlsx without openpyxl."""
import argparse
import re
import sys
import zipfile
...
DEFAULT_OUTPUT = Path("docs/references/extracts/jcsf")
...
def rows_to_markdown(sheet_name: str, rows: list[list[str]]) -> str:
    lines = [f"# JCSF extract: {sheet_name}", "", f"Source: `JCSF v7_public.xlsx` sheet `{sheet_name}`.", ""]
    if not rows:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Extract JCSF xlsx sheet rows")
    ...
    if not args.xlsx or not args.xlsx.exists():
        print('Pass --xlsx "/path/to/JCSF v7_public.xlsx" (vendor file, not in git)', file=sys.stderr)
        sys.exit(1)
